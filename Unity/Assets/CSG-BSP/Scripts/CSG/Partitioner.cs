...
namespace CSG {
    public class Partitioner {
        ...
        private static Vector3D tempVector3D = new Vector3D();
        private static Vertex tempVertex = new Vertex();
            ...
            switch (triOrientation) {
                ...
                case Orientation.Split:
                    ...
                    for (int i = 0; i < 3; i++) {
                        ...
                        Vector3D currentEdge = nextVertex.Position.SubtractedBy(currentVertex.Position, tempVector3D);
                        ...
                        if (currentOrientation == Orientation.GreaterThan && nextOrientation == Orientation.LessThan ||
                           currentOrientation == Orientation.LessThan && nextOrientation == Orientation.GreaterThan) {
                            ...
                            Vertex splitVertex = currentVertex.Lerped(nextVertex, splitFraction, tempVertex);
                            gtSplit.Add(new Vertex(splitVertex));
                            ltSplit.Add(new Vertex(splitVertex));
                        }
                    }
